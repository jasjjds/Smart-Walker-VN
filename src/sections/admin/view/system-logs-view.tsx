    ...
    <div className="w-full h-full flex flex-col gap-6 text-[#0c4a6e]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <button className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 border-2 border-[#bae6fd] hover:bg-[#bae6fd]/30 text-[#0c4a6e] font-semibold rounded-lg transition-colors shadow-sm text-xs sm:text-sm">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
            Làm mới
          </button>
          <button className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 bg-[#0ea5e9] hover:bg-[#0284c7] text-white font-semibold rounded-lg transition-colors shadow-md text-xs sm:text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 shrink-0">
        ...
        {/* Khối Thống kê chung */}
        <div className="bg-[#e0f2fe] rounded-2xl p-5 sm:p-6 border border-[#bae6fd] shadow-sm flex flex-col justify-between">
          <h3 className="text-base sm:text-lg font-bold mb-4">Tổng quan hôm nay</h3>
          <div className="space-y-4">
            <div className="flex justify-between items-center pb-3 border-b border-[#0c4a6e]/10 text-xs sm:text-sm">
        ...
        {/* Biểu đồ Cột */}
        <div className="bg-[#e0f2fe] rounded-2xl p-5 sm:p-6 border border-[#bae6fd] shadow-sm lg:col-span-2">
          ...
          <div className="h-32 w-full flex items-end justify-between gap-2 md:gap-4 px-2">
            {activityData.map((data, index) => (
              <div key={index} className="flex flex-col items-center flex-1 group">
                <span className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] sm:text-xs font-bold mb-1 text-[#0ea5e9]">{data.count}</span>
                <div
                  className="w-full max-w-[32px] sm:max-w-[40px] bg-gradient-to-t from-[#0ea5e9] to-[#7dd3fc] rounded-t-md transition-all duration-500 group-hover:from-[#0284c7] group-hover:to-[#0ea5e9]"
                  style={{ height: data.height }}
                ></div>
                <span className="text-[10px] sm:text-xs font-semibold mt-2 text-[#0c4a6e]/70">{data.day}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#e0f2fe] rounded-2xl shadow-sm border border-[#bae6fd] flex flex-col flex-1 overflow-hidden mt-2">
        ...
        {/* Bộ lọc bảng */}
        <div className="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[#0c4a6e]/10">
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold">Lịch sử sự kiện</h2>

          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <select className="px-4 py-2.5 bg-white rounded-lg border border-[#bae6fd] focus:outline-none focus:ring-2 focus:ring-[#0ea5e9] text-[#0c4a6e] font-medium shadow-sm cursor-pointer text-xs sm:text-sm">
            ...
            <div className="relative w-full sm:w-64">
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white rounded-lg border border-[#bae6fd] focus:outline-none focus:ring-2 focus:ring-[#0ea5e9] text-[#0c4a6e] placeholder-[#0c4a6e]/50 font-medium shadow-sm transition-shadow text-xs sm:text-sm"
              />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-[#e0f2fe] text-[#0c4a6e] text-xs sm:text-sm md:text-base border-b border-[#0c4a6e]/20">
            ...
            <tbody>
              {mockLogs.map((log) => (
                <tr key={log.id} className="border-b border-[#f0f9ff]/50 hover:bg-[#bae6fd]/30 transition-colors">
                  ...
                  <td className="py-4 px-6 font-bold text-[#0c4a6e]/80 text-xs sm:text-sm">
                    {log.source}
                  </td>
                  <td className="py-4 px-6 font-medium text-xs sm:text-sm">
                    {log.message}
                  </td>
                  <td className="py-4 px-6 font-mono text-[10px] sm:text-xs text-[#0c4a6e]/60">
                    {log.ip}
                  </td>
                </tr>
              ))}
        ...
        {/* Phân trang */}
        <div className="px-4 py-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between border-t border-[#0c4a6e]/10 bg-[#e0f2fe]/50 gap-3">
          <span className="text-xs sm:text-sm font-medium text-[#0c4a6e]/70">Hiển thị 1 - 5 của 2,451 kết quả</span>
