  ...
  return (
    <div className="w-full h-full flex flex-col gap-6 text-[#0c4a6e]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Phân tích Trọng tâm (CoG)</h1>
          <p className="text-[#0c4a6e]/70 mt-1 text-sm font-medium italic">
            Mô phỏng sự dịch chuyển trọng tâm của bệnh nhân trong không gian 2D.
          </p>
        </div>
        <button
          onClick={simulateMovement}
          className="w-full sm:w-auto px-6 py-2.5 bg-[#0ea5e9] hover:bg-[#0c4a6e] text-white font-bold rounded-xl shadow-md transition-all text-sm"
        >
          Mô phỏng bước đi
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-[#e0f2fe] rounded-3xl p-6 sm:p-8 border border-[#bae6fd] shadow-sm flex flex-col items-center">
          <div className="relative w-full max-w-[320px] xs:max-w-[360px] sm:max-w-[400px] aspect-square flex items-center justify-center my-6">
            {/* Nhãn hướng */}
            <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] sm:text-xs font-black uppercase text-[#0c4a6e]/40">Trước (Forward)</span>
            <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] sm:text-xs font-black uppercase text-[#0c4a6e]/40">Sau (Backward)</span>
            <span className="absolute top-1/2 -left-8 sm:-left-12 -translate-y-1/2 text-[10px] sm:text-xs font-black uppercase text-[#0c4a6e]/40">Trái</span>
            <span className="absolute top-1/2 -right-8 sm:-right-12 -translate-y-1/2 text-[10px] sm:text-xs font-black uppercase text-[#0c4a6e]/40">Phải</span>

            {/* Vẽ SVG Target (Mềm dẻo và co giãn theo kích cỡ bình thường) */}
            <svg 
              viewBox={`0 0 ${SVG_SIZE} ${SVG_SIZE}`} 
              className="w-full h-full bg-white rounded-full shadow-inner border border-[#bae6fd]"
            >
          ...
          <div className="mt-4 grid grid-cols-2 gap-8 w-full max-w-sm">
            <div className="text-center">
              <p className="text-[10px] sm:text-xs font-bold text-[#0c4a6e]/50 uppercase">Tọa độ X (cm)</p>
              <p className="text-xl sm:text-2xl font-black">{currentPoint.x > 0 ? `+${currentPoint.x}` : currentPoint.x}</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] sm:text-xs font-bold text-[#0c4a6e]/50 uppercase">Tọa độ Y (cm)</p>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-[#0c4a6e] rounded-3xl p-6 sm:p-8 text-white flex flex-col items-center shadow-xl">
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#bae6fd] shadow-sm flex-1">
            <h3 className="text-base sm:text-lg font-bold mb-4">Chẩn đoán sơ bộ</h3>
            <div className="space-y-4">
              <div className="p-3 sm:p-4 bg-[#f0f9ff] rounded-2xl">
                <p className="text-[10px] sm:text-xs font-bold text-[#0ea5e9] uppercase mb-1">Xu hướng lệch</p>
                <p className="font-bold text-sm sm:text-base">
                  {currentPoint.x > 2 ? "Nghiêng sang Phải" : currentPoint.x < -2 ? "Nghiêng sang Trái" : "Cân bằng ngang"}
                </p>
              </div>
              <div className="p-3 sm:p-4 bg-[#f0f9ff] rounded-2xl">
                <p className="text-[10px] sm:text-xs font-bold text-[#0ea5e9] uppercase mb-1">Tư thế đứng</p>
                <p className="font-bold text-sm sm:text-base">
                  {currentPoint.y > 2 ? "Ngả người về trước" : currentPoint.y < -2 ? "Ngả người ra sau" : "Tư thế thẳng"}
                </p>
              </div>
            </div>
            <button className="w-full mt-6 py-3 border-2 border-[#bae6fd] text-[#0c4a6e] font-bold rounded-xl hover:bg-[#bae6fd]/30 transition-colors text-sm">
              Xuất báo cáo PDF
            </button>
          </div>
        </div>
