...
export function ProfileDrawer({ isOpen, onClose }: ProfileDrawerProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      ...
      {/* Drawer panel */}
      <aside className={`absolute top-0 right-0 w-80 sm:w-96 h-full bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} text-primary-900`}>
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center">
          {/* Avatar block */}
          <div className="w-24 h-24 rounded-full bg-primary-500/10 border-4 border-primary-500/20 flex items-center justify-center text-primary-500 text-3xl font-black mb-4 shrink-0">
            {getInitials(user?.full_name)}
          </div>

          <span className="px-3 py-1 bg-primary-500/10 text-primary-500 rounded-full text-xs font-bold uppercase tracking-wider mb-2 shrink-0">
            {getRoleLabel(user?.role_id)}
          </span>

          <div className="flex flex-col items-center gap-0.5 mb-4 shrink-0 text-primary-900 font-mono text-sm sm:text-base">
            {user?.patient_id ? (
              <span className="font-bold">Mã bệnh nhân: {user.patient_id}</span>
            ) : (
          ...
          {isEditing ? (
            <form onSubmit={handleSaveProfile} className="w-full flex flex-col gap-4 border-t border-slate-100 pt-6">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-primary-900/50 uppercase tracking-wider font-sans">Họ và tên</label>
                <input
                  type="text"
                  value={editFullName}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-primary-900/50 uppercase tracking-wider font-sans">Email đăng nhập</label>
                <input
                  type="email"
                  value={user?.email || ""}
                  disabled
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-primary-900/50 uppercase tracking-wider font-sans">Số điện thoại</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-primary-900/50 uppercase tracking-wider font-sans">Giới tính</label>
                <select
                  value={editGender}
              ...
              <div className="flex items-center gap-2 mt-4">
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-1 py-2 bg-primary-500 hover:bg-primary-500/90 text-white font-bold rounded-lg transition-colors text-sm disabled:opacity-50 cursor-pointer text-center"
                >
            ...
            <div className="w-full flex flex-col gap-5 border-t border-slate-100 pt-6">
              ...
              <div className="flex flex-col gap-2 mt-4">
                <button
                  ...
                  className="w-full py-2.5 bg-primary-500/10 hover:bg-primary-500/20 text-primary-500 font-bold rounded-xl transition-colors flex items-center justify-center gap-2 text-sm cursor-pointer"
                >
