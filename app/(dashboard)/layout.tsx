              ...
              <div key={group.id} className={groupIndex > 0 ? "mt-2" : ""}>
                ...
                >
                  {group.items.map((item) => {
                    // Thay vì dùng startsWith thuần túy, ta dùng một hàm kiểm tra thông minh hơn
                    const isActive =
                      item.path === '/dashboard' || item.path === '/dashboard/doctor'
                        // Những link được coi là "Gốc" thì bắt buộc phải trùng khớp hoàn toàn (Exact Match)
                        ? pathname === item.path
                        // Những link con thì kiểm tra xem nó có đúng là đường dẫn đó không, hoặc có phải là thư mục con không
