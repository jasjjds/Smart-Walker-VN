...
import { useParams } from "next/navigation"; // 1. Import hook lấy params
...
export default function DoctorDashboardPage() {
  // 2. Lấy id từ URL (ví dụ: /dashboard/patients/5 => patientId = "5")
    ...
    <div className="w-full h-full flex flex-col gap-8 text-[#0c4a6e]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {clinicalMetrics.map((metric) => (
          <Link
            key={metric.title}
            // 3. Gắn thêm patient_id vào đuôi URL để chuyển tiếp sang trang biểu đồ
            href={`${patientId}/${metric.path}`}
